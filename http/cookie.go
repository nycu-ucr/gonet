...
func (c *Cookie) String() string {
	...
	if len(c.Domain) > 0 {
		if validCookieDomain(c.Domain) {
			...
			log.Printf("github.com/nycu-ucr/gonet/http: invalid Cookie.Domain %q; dropping domain attribute", c.Domain)
		}
	}
...
func sanitizeOrWarn(fieldName string, valid func(byte) bool, v string) string {
	...
	for i := 0; i < len(v); i++ {
		...
		log.Printf("github.com/nycu-ucr/gonet/http: invalid byte %q in %s; dropping invalid bytes", v[i], fieldName)
		ok = false
		break
	}
	if ok {
