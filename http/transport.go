...
func (t *Transport) roundTrip(req *Request) (*Response, error) {
	...
	if isHTTP {
		for k, vv := range req.Header {
			if !httpguts.ValidHeaderFieldName(k) {
				req.closeBody()
				return nil, fmt.Errorf("github.com/nycu-ucr/gonet/http: invalid header field name %q", k)
			}
			for _, v := range vv {
				if !httpguts.ValidHeaderFieldValue(v) {
					req.closeBody()
					// Don't include the value in the error, because it may be sensitive.
					return nil, fmt.Errorf("github.com/nycu-ucr/gonet/http: invalid header field value for %q", k)
				}
			}
		}
	}
	...
	if req.Method != "" && !validMethod(req.Method) {
		req.closeBody()
		return nil, fmt.Errorf("github.com/nycu-ucr/gonet/http: invalid method %q", req.Method)
	}
	if req.URL.Host == "" {
		req.closeBody()
...
var errCannotRewind = errors.New("github.com/nycu-ucr/gonet/http: cannot rewind body after connection loss")
...
var ErrSkipAltProtocol = errors.New("github.com/nycu-ucr/gonet/http: skip alternate protocol")
...
func (e transportReadFromServerError) Error() string {
	return fmt.Sprintf("github.com/nycu-ucr/gonet/http: Transport failed to read from server: %v", e.err)
}
...
func (t *Transport) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	// println("Transport dial")
	if t.DialContext != nil {
		// println("Transport DialContext")
	...
	if t.Dial != nil {
		...
		if c == nil && err == nil {
			err = errors.New("github.com/nycu-ucr/gonet/http: Transport.Dial hook returned (nil, nil)")
		}
		return c, err
	}
...
func (w *wantConn) tryDeliver(pc *persistConn, err error) bool {
	...
	w.err = err
	if w.pc == nil && w.err == nil {
		panic("github.com/nycu-ucr/gonet/http: internal error: misuse of tryDeliver")
	}
	close(w.ready)
	return true
}
...
func (t *Transport) customDialTLS(ctx context.Context, network, addr string) (conn net.Conn, err error) {
	...
	if conn == nil && err == nil {
		err = errors.New("github.com/nycu-ucr/gonet/http: Transport.DialTLS or DialTLSContext returned (nil, nil)")
	}
	return
}
...
func (t *Transport) decConnsPerHost(key connectMethodKey) {
	...
	if n == 0 {
		...
		panic("github.com/nycu-ucr/gonet/http: internal error: connCount underflow")
	}
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (pconn *persistConn, err error) {
	...
	if cm.scheme() == "https" && t.hasCustomTLSDialer() {
		...
		println("\u001b[33m You are using ONVM \u001b[0m, gonet/http/transport.go")
		conn, err := onvmpoller.DialONVM("onvm", cm.addr())
		if err != nil {
			return nil, wrapErr(err)
		}
		pconn.conn = conn
...
func (pc *persistConn) mapRoundTripError(req *transportRequest, startBytesWritten int64, err error) error {
	...
	if pc.isBroken() {
		...
		return fmt.Errorf("github.com/nycu-ucr/gonet/http: HTTP/1.x transport connection broken: %v", err)
	}
	return err
}
...
func (pc *persistConn) readLoop() {
	...
	for alive {
		...
		if err != nil {
			if pc.readLimit <= 0 {
				err = fmt.Errorf("github.com/nycu-ucr/gonet/http: server response headers exceeded %d bytes; aborted", pc.maxHeaderResponseSize())
			}
	...
	for {
		...
		if is1xxNonTerminal {
			...
			if num1xx > max1xxResponses {
				return nil, errors.New("github.com/nycu-ucr/gonet/http: too many 1xx informational responses")
			}
...
var errTimeout error = &httpError{err: "github.com/nycu-ucr/gonet/http: timeout awaiting response headers", timeout: true}

// errRequestCanceled is set to be identical to the one from h2 to facilitate
// testing.
var errRequestCanceled = http2errRequestCanceled
var errRequestCanceledConn = errors.New("github.com/nycu-ucr/gonet/http: request canceled while waiting for connection") // TODO: unify?
...
func (tlsHandshakeTimeoutError) Error() string {
	return "github.com/nycu-ucr/gonet/http: TLS handshake timeout"
}
